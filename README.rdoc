...
This is a PRE-RELEASE VERSION.  Tests & better docs pending.
...
* add some css/sass styling ... TODO...
* Create a config file in config/barkeep.json (see example below)
* call render_barkeep from your layout or footer, i.e. <%= render_barkeep %> in
  erb or just =render_barkeep in Haml

== Config

Configuration is specified in a config/barkeep.json

At minimum, you want to specify some panes and a github url:
...
ApplicationHelper.  Alternatively, you can specify a partial using the format
"partial path/to/your/partial"
